...
for i in range(len(df)):
	...
	plt.errorbar(ml, y = mean, yerr = confint, marker='.', fmt='.', solid_capstyle="projecting", capsize = 5, c = colors[c_index]\
		, label='t0 = {}'.format(t0))
...
for i in range(len(df)):
	...
	if t0 != 0:
		plt.errorbar(t0, y = mean, yerr = confint, marker='.', fmt='.', solid_capstyle="projecting", capsize = 5, c = colors[c_index]\
			, label='Markov multiplier = {:.1f}'.format((ml)))
